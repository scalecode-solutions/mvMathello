...
/// Drives the solo Spiral capstone: ghost aiming, placement, the diagonal
/// progression, and the burst. Untimed.
@MainActor
@Observable
public final class SpiralViewModel {
    public private(set) var session: SpiralSession
    ...
    public init(rows: Int = 12, seed: UInt64 = 0x5EED) {
        let session = SpiralSession(rows: rows, seed: seed)
    ...
    public func placeGhost() {
        ...
        let before = session.completedDiagonals
        ...
        orientationIndex = 0
        Haptics.play(session.completedDiagonals > before ? .golden : .place)
        if session.isOver { Haptics.play(.gameOver) }
    }

    public func newGame(seed: UInt64) {
        session = SpiralSession(rows: session.board.rows, seed: seed)
        orientationIndex = 0
        lastBurst = nil
    }
