...
/// Chains — the solo combo puzzle. Snake one long connected chain of pentominoes
/// to climb the ×φ multiplier on your Collatz score.
public struct ChainsGameView: View {
    @Environment(\.mathelloTheme) private var theme
    @State private var vm: ChainsViewModel
    private let embedded: Bool

    public init(rows: Int = 12, seed: UInt64 = 0x5EED, embedded: Bool = false) {
        _vm = State(initialValue: ChainsViewModel(rows: rows, seed: seed))
    ...
    private var statusBar: some View {
        HStack {
            stat("SCORE", "\(vm.session.score)", theme.headlineColor)
            Spacer()
            stat("CHAIN", "\(vm.session.chainLength)", theme.golden)
            Spacer()
            stat("MULT", String(format: "×%.1f", vm.multiplier), theme.golden)
        }
    }
    ...
    private var dropReadout: some View {
        ...
        return Group {
            if !vm.ghostLegal {
                Text("off the board").foregroundStyle(theme.bodyColor.opacity(0.6))
            } else if vm.ghostExtends {
                Text("links chain  +\(score)").foregroundStyle(theme.golden)
            } else {
                Text("new chain  +\(score)").foregroundStyle(theme.bodyColor)
            }
        }
